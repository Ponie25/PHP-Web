...
<section class="mt-4">
    <?php
    $stmt = $pdo->query("SELECT title, content, author, likes, comments, bookmarks FROM posts ORDER BY created_at DESC");
    while ($post = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo "<div class='bg-white shadow-lg rounded-lg p-4 mb-4 w-full'>";
        echo "<h2 class='text-xl font-bold'>{$post['title']}</h2>";
        echo "<p>{$post['content']}</p>";
        echo "<p class='text-gray-500 text-sm'>Posted by {$post['author']}</p>";
    ...
    <div class="bg-white p-6 rounded-lg w-1/3">
        ...
        <form action="/PHP-Web-main/PHP/upload_post.php" method="POST">
            ...
            <div class="flex space-x-4">
                ...
                <input type"text" name="author" placeholder="Author" class="w-full p-2 border mb-4">
            </div>
            <!-- Content -->
